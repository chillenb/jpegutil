...
Jpeg::Jpeg(vector<byte> data) { buf = data; }
...
Jpeg Jpeg::loadFromFile(const string &filename) {
  ...
  if (tjDecompressHeader3(tjInstance, (const unsigned char *)&newjpeg.buf[0],
                          newjpeg.buf.size(), &newjpeg.w, &newjpeg.h,
...
int myfunc(float a, short b, tjregion c) { return 0; }
...
void Jpeg::loadDctCoeffs() {
  ...
  Callback<int(short *, tjregion, tjregion, int, int, tjtransform *)>::func =
      std::bind(&Jpeg::Dctcoeffs::dummyFilter, _coeffs, _1, _2, _3, _4, _5, _6);

  int (*customFilterCallback)(short *, tjregion, tjregion, int, int,
                              tjtransform *) =
      static_cast<decltype(customFilterCallback)>(
          Callback<int(short *, tjregion, tjregion, int, int,
                       tjtransform *)>::callback);
  ...
  if (tjTransform(tjInstance, (const unsigned char *)&buf[0], buf.size(), 1,
                  &dstBuf, &dstSize, &xform, TJFLAG_ACCURATEDCT) < 0) {
    tjFree(dstBuf);
  }
  tjFree(dstBuf);
  tjDestroy(tjInstance);
...
int Jpeg::Dctcoeffs::dummyFilter(short *coeffs, tjregion arrayRegion,
  ...
  int i;
  printf("%d x %d, (%d x %d)\n", arrayRegion.w, arrayRegion.h, arrayRegion.x,
         arrayRegion.y);
